	...
	else
	{
		...
		if (pEntity)
		{
			...
			if ((m_flNextPrimaryAttack + 1 < UTIL_WeaponTimeBase()) || g_pGameRules->IsMultiplayer())
			{
				...
				pEntity->TraceAttack(m_pPlayer->pev, GetSkillFloat("sk_plr_pipewrench"sv), gpGlobals->v_forward, &tr, DMG_CLUB);
			}
			else
			{
				// subsequent swings do half
				pEntity->TraceAttack(m_pPlayer->pev, GetSkillFloat("sk_plr_pipewrench"sv) / 2, gpGlobals->v_forward, &tr, DMG_CLUB);
			}
...
void CPipewrench::BigSwing()
{
	...
	else
	{
		...
		if (pEntity)
		{
			...
			float flDamage = (gpGlobals->time - m_flBigSwingStart) * GetSkillFloat("sk_plr_pipewrench"sv) + 25.0f;
			if ((m_flNextPrimaryAttack + 1 < UTIL_WeaponTimeBase()) || g_pGameRules->IsMultiplayer())
			{
				// first swing does full damage
