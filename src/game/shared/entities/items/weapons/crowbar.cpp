...
bool CCrowbar::Swing(bool fFirst)
{
	...
	else
	{
		...
		if ((m_flNextPrimaryAttack + 1 < UTIL_WeaponTimeBase()) || g_pGameRules->IsMultiplayer())
		{
			...
			pEntity->TraceAttack(m_pPlayer->pev, GetSkillFloat("sk_plr_crowbar"sv), gpGlobals->v_forward, &tr, DMG_CLUB);
		}
		else
		{
			// subsequent swings do half
			pEntity->TraceAttack(m_pPlayer->pev, GetSkillFloat("sk_plr_crowbar"sv) / 2, gpGlobals->v_forward, &tr, DMG_CLUB);
		}
