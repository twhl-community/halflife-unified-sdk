...
typedef struct pmtrace_s* (*pfnEngSrc_PM_TraceLine_t)(const float* start, const float* end, int flags, int usehull, int ignore_pe);
...
typedef void (*pfnEngSrc_pfnPlaySoundByNameAtLocation_t)(const char* szSound, float volume, const float* origin);
