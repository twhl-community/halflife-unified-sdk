...
void DisplayTeamFlags(CBaseEntity* pPlayer)
{
	...
	{
		auto player = (CBasePlayer*)UTIL_PlayerByIndex(i);
		...
		if (team1Flag || team2Flag)
		{
			...
			if (team1Flag && gmsgFlagIcon)
			{
				g_engfuncs.pfnMessageBegin(MSG_ONE, gmsgFlagIcon, 0, player->edict());
				g_engfuncs.pfnWriteByte(1);

				switch (team1Flag->m_iGoalState)
				{
				case 2:
					if (team1Flag == pMyFlag && pMyFlag)
...
void CTFGoalFlag::goal_item_dropthink()
{
	...
	else
	{
		...
		pev->nextthink = gpGlobals->time + max(0, g_flFlagReturnTime - 5.0);
	}
}
